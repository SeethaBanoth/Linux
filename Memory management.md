...
int main() {
   ...
   if (ptr == NULL) {
       ...
       printf("calloc() failed - insufficient memory!n");
   ...
   printf("Memory allocated and initialized to zero.n");
...
- `realloc(old_ptr, new_size)` resizes **in-place** if possible
...
int main() {
   ...
   printf("Memory Blocks:t");

   for (int i = 0; i < m; i++)

       printf("%dt", blockSize[i]);

   printf("nProcess Size:t");

   for (int i = 0; i < n; i++)

       printf("%dt", processSize[i]);
   ...
   printf("Process No.tProcess SizetBlock No.n");

   for (int i = 0; i < n; i++) {

       printf("   %dtt   %dtt", (i+1), processSize[i]);
...
int main() {
   ...
   int next_idx = 0;  // Starting point for next-fit
   ...
   for (int i = 0; i < n; i++) {
       ...
       while (1) {
           ...
           if (blockSize[next_idx] >= processSize[i]) {

               allocation[i] = next_idx;

               blockSize[next_idx] -= processSize[i];

               next_idx = (next_idx + 1) % m;  // Move to next block
           ...
           if (next_idx == start_idx) break;  // Full circle - no fit
...
#define MAX_ORDER 10  // 2^10 = 1024 units max block
...
int is_free[BLOCK_SIZE / 2][MAX_ORDER + 1];  // Free list for each order
...
void init_memory() {
   ...
   memset(is_free, 1, sizeof(is_free));  // Mark all blocks free

   is_free[0][MAX_ORDER] = 1;  // Root block is free

}



int find_free_block(int order) {

   for (int i = 0; i < BLOCK_SIZE >> MAX_ORDER; i++) {
...
int allocate(int size) {
   ...
   int block = find_free_block(order);

   if (block == -1) {

       printf("Allocation failed: No free block of size %dn", 1 << order);
   ...
   while (order > 0) {
       ...
       int left_child = block;

       int right_child = block + (1 << (order - 1));

       

       is_free[left_child >> MAX_ORDER][order - 1] = 1;

       is_free[right_child >> MAX_ORDER][order - 1] = 1;

       is_free[block >> MAX_ORDER][order] = 0;

       

       order--;

       block = left_child;  // Use left half
   ...
   printf("Allocated %d bytes at address %dn", 1 << order, block);
...
void deallocate(int block_addr) {

   if (memory[block_addr] == 0) {

       printf("Block %d already freen", block_addr);
   ...
   while (order < MAX_ORDER && memory[block_addr + (1 << order)] == 0) {
   ...
   is_free[block_addr >> MAX_ORDER][order] = 1;

   

   // Try to merge with buddy

   int buddy = block_addr ^ (1 << order);

   if (buddy < BLOCK_SIZE && memory[buddy] == 0) {
   ...
   printf("Freed block at %dn", block_addr);
...
Block mem_blocks[MAX_BLOCKS];

int block_count = 0;

int memory_pool[MAX_POOL];



// Custom "Quick-Fit" algorithm: separate lists for common sizes

int free_16[MAX_BLOCKS], free_32[MAX_BLOCKS], free_64[MAX_BLOCKS];

int idx_16 = 0, idx_32 = 0, idx_64 = 0;
...
void init_allocator() {
   ...
   mem_blocks[0].size = MAX_POOL;

   mem_blocks[0].is_free = 1;

   block_count = 1;

   

   printf("Custom Memory Allocator Initialized (Pool: %d bytes)n", MAX_POOL);

}



int quick_fit_alloc(int size) {

   // Quick-fit for common sizes

   if (size <= 16 && idx_16 > 0) {

       int block_idx = free_16[--idx_16];

       mem_blocks[block_idx].is_free = 0;

       printf("Quick-alloc %d bytes at %d (16-byte bin)n", size, mem_blocks[block_idx].start);
   ...
   if (size <= 32 && idx_32 > 0) {
       ...
       int block_idx = free_32[--idx_32];

       mem_blocks[block_idx].is_free = 0;

       printf("Quick-alloc %d bytes at %d (32-byte bin)n", size, mem_blocks[block_idx].start);
   ...
   if (size <= 64 && idx_64 > 0) {
       ...
       int block_idx = free_64[--idx_64];

       mem_blocks[block_idx].is_free = 0;

       printf("Quick-alloc %d bytes at %d (64-byte bin)n", size, mem_blocks[block_idx].start);
   ...
   for (int i = 0; i < block_count; i++) {
       ...
       if (mem_blocks[i].is_free && mem_blocks[i].size >= size) {
           ...
           if (mem_blocks[i].size > size + 16) {

               Block new_block;

               new_block.start = mem_blocks[i].start + size;

               new_block.size = mem_blocks[i].size - size;
               ...
               mem_blocks[block_count++] = new_block;
           ...
           printf("First-fit alloc %d bytes at %dn", size, mem_blocks[i].start);
...
void quick_fit_free(int block_idx) {

   if (block_idx < 0 || block_idx >= block_count || mem_blocks[block_idx].is_free) {

       printf("Invalid free requestn");
   ...
   int size = mem_blocks[block_idx].size;

   mem_blocks[block_idx].is_free = 1;

   

   // Add to quick-fit bins

   if (size <= 16 && idx_16 < MAX_BLOCKS) {

       free_16[idx_16++] = block_idx;

   } else if (size <= 32 && idx_32 < MAX_BLOCKS) {

       free_32[idx_32++] = block_idx;

   } else if (size <= 64 && idx_64 < MAX_BLOCKS) {

       free_64[idx_64++] = block_idx;
   ...
   for (int i = 0; i < block_count - 1; i++) {
       ...
       if (mem_blocks[i].is_free && mem_blocks[i+1].is_free) {
   ...
   printf("Freed block %d (size %d)n", mem_blocks[block_idx].start, size);
...
void print_status() {
   ...
   printf("nMemory Status:n");

   printf("AddrtSizetStatusn");

   for (int i = 0; i < block_count; i++) {

       printf("%dt%dt%sn", mem_blocks[i].start, 

              mem_blocks[i].size, 

              mem_blocks[i].is_free ? "FREE" : "USED");

   }

   printf("Quick-fit bins: 16B=%d, 32B=%d, 64B=%dnn", 

          idx_16, idx_32, idx_64);
...
int main() {
   ...
   int b1 = quick_fit_alloc(24);

   int b2 = quick_fit_alloc(40);

   int b3 = quick_fit_alloc(16);
   ...
   int b4 = quick_fit_alloc(32);  // Should use quick-fit
...
int main() {
   ...
   fd = open("mmap_demo.txt", O_CREAT | O_RDWR, 0644);
   ...
   mapped_memory = mmap(NULL, 4096, PROT_READ | PROT_WRITE, 
   ...
   printf("File mapped to memory at address: %pn", mapped_memory);

   

   // Write to mapped memory (changes go to file)

   strcpy(mapped_memory, message);

   printf("Written to memory: %sn", mapped_memory);

   

   // Read from mapped memory

   printf("Read from memory: %sn", mapped_memory);

   

   // Changes are automatically flushed to disk

   printf("Check file 'mmap_demo.txt' - data is persisted!n");
...
Check file 'mmap_demo.txt' - data is persisted!
...
- `MAP_SHARED`: Changes persist to file
...
int main() {
   ...
   mapped_mem = mmap(NULL, 4096, PROT_READ | PROT_WRITE, 
   ...
   printf("File mapped at: %p (size: %ld bytes)n", mapped_mem, file_stat.st_size);
   ...
   printf("Original content: %.100s...n", mapped_mem);
   ...
   sprintf(mapped_mem, "Memory mapped file demo! Time: %ld", time(NULL));

   printf("Written: %sn", mapped_mem);

   

   // MODIFY specific offset

   strcpy(mapped_mem + 50, " [MODIFIED]");

   printf("After modification: %.100s...n", mapped_mem);

   

   // Verify file persistence

   printf("n=== PERSISTENCE CHECK ===n");
   ...
   mapped_mem = mmap(NULL, 4096, PROT_READ, MAP_SHARED, fd, 0);

   printf("File after unmap/reopen: %sn", mapped_mem);
...
- **Write**: Changes automatically sync to disk (`MAP_SHARED`)

- **Random access**: `mapped_mem + offset` for seeking
...
int main() {
   ...
   printf("Shared memory attached at %pn", shared_memory);
   ...
   if (pid == 0) {  // Child process - WRITER
       ...
       printf("[CHILD %d] Writing to shared memory...n", getpid());

       strcpy(shared_memory, "Hello from Child Process!");

       printf("[CHILD %d] Written: %sn", getpid(), shared_memory);
       ...
       printf("[PARENT %d] Reading from shared memory: %sn", getpid(), shared_memory);
       ...
       printf("[PARENT %d] Updated shared memoryn", getpid());
       ...
       printf("[PARENT %d] Child finished. Final content: %sn", getpid(), shared_memory);
...
int main() {
   ...
   printf("Shared memory created, semaphore initializedn");
   ...
   if (pid == 0) {  // Child process
       ...
       for (int i = 0; i < 5; i++) {
           ...
           printf("[CHILD %d] Counter: %d, Message: %sn", 

                  getpid(), shared_data->counter, shared_data->message);
           ...
           sprintf(shared_data->message, "Modified by CHILD %d", getpid());
       ...
       for (int i = 0; i < 5; i++) {
           ...
           printf("[PARENT %d] Counter: %d, Message: %sn", 
           ...
           sprintf(shared_data->message, "Modified by PARENT %d", getpid());
       ...
       printf("nCleanup complete. Final counter: %dn", shared_data->counter);
...
int fifo(int frames[], int ref[], int n, int capacity) {
   ...
   for (int i = 0; i < n; i++) {
       ...
       if (!found) {
           ...
           frame_count = (frame_count + 1) % capacity;
...
int lru(int ref[], int n, int capacity) {
   ...
   int frame_count = 0, page_faults = 0;
   ...
   for (int i = 0; i < n; i++) {
       ...
       if (!found && frame_count < capacity) {

           frames[frame_count] = ref[i];

           counters[frame_count] = i;
           ...
           int min_counter = counters[0], min_index = 0;

           for (int j = 1; j < capacity; j++) {

               if (counters[j] < min_counter) {

                   min_counter = counters[j];
   ...
   for (int i = 0; i < n; i++) {
       ...
       if (!found && frame_count < capacity) {
           ...
           for (int j = 0; j < frame_count; j++) {
               ...
               if (future_use == -1 || future_use > farthest) {
...
int main() {
   ...
   int fifo_faults = fifo(frames, ref, n, capacity);

   printf("Page Faults: %dn", fifo_faults);
   ...
   int lru_faults = lru(ref, n, capacity);

   printf("Page Faults: %dn", lru_faults);
   ...
   int opt_faults = optimal(ref, n, capacity);

   printf("Page Faults: %dn", opt_faults);

   

   printf("nSummary:n");

   printf("FIFO: %d faultsn", fifo_faults);

   printf("LRU: %d faultsn", lru_faults);

   printf("Optimal: %d faultsn", opt_faults);
